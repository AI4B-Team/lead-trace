#!/usr/bin/env bun
// ---------------------------------------------------------------------------
// Round 3: find the clerk's records desk by following the site's own links.
//
//   bun run scripts/discover-fl-clerk-contacts3.ts
//
...
const LINKY = /contact|public\s*record|records\s*request|tax\s*deed|surplus|unclaimed|excess|finance|comptroller|custodian/i;
...
async function main() {
  ...
  writeFileSync("reports/fl-clerk-contacts-round3.json", JSON.stringify({ generatedAt: new Date().toISOString(), report }, null, 2));
